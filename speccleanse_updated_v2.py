...
def print_result(result: ProcessingResult, verbose: bool = False, 
    ...
    if deep_result:
        ...
        print(f"    RSID registry blocks: {getattr(deep_result, 'rsid_registry_removed', 0)}")
        ...
        print(f"    Compat elements:     {getattr(deep_result, 'compat_elements_removed', 0)}")
        print(f"    Internal bookmarks:  {deep_result.bookmarks_removed}")
        print(f"    Proof state:         {deep_result.proof_elements_removed}")
        print(f"    External link rels:  {getattr(deep_result, 'external_link_rels_removed', 0)}")
        print(f"    Hyperlinks unwrapped:{getattr(deep_result, 'hyperlink_elements_unwrapped', 0)}")
        print(f"    app.xml HLinks rm:   {getattr(deep_result, 'app_hlinks_removed', 0)}")
        print()
...
def run_deep_clean(unpacked_dir: Path, args, verbose: bool = False):
    ...
    if args.dry_run:
        ...
        result.rsid_registry_removed = 0
        result.compat_elements_removed = 0
        result.external_link_rels_removed = 0
        result.hyperlink_elements_unwrapped = 0
        result.app_hlinks_removed = 0
    ...
    if only:
        ...
        flags = {
            ...
            'remove_rsid_registry': False,
            'aggressive_compat': False,
            'scrub_external_links': False,
            'scrub_link_domains': [],
        }
        ...
        only_map = {
            ...
            'rsidreg': 'remove_rsid_registry',
            'links': 'scrub_external_links',
        }
        if only in only_map:
            flags[only_map[only]] = True
            if only == 'links':
                flags['scrub_link_domains'] = args.strip_links_domain or []
            if only == 'compat':
                flags['aggressive_compat'] = args.aggressive_compat
        ...
        flags = {
            ...
            'aggressive_compat': args.aggressive_compat,
            'scrub_external_links': not args.no_links,
            'scrub_link_domains': args.strip_links_domain or [],
        }
...
Deep Clean (--deep):
  ...
  • Proof state (spell/grammar check markers)
    ...
    deep_group.add_argument(
        "--only",
        choices=['media', 'styles', 'rsids', 'rsidreg', 'empty', 'fonts', 'compat', 'links', 'bookmarks', 'proof'],
    ...
    deep_group.add_argument(
        "--aggressive-compat",
        action="store_true",
        help="Aggressively remove entire <w:compat> blocks from settings.xml (higher risk)"
    )
    ...
    deep_group.add_argument(
        ...
        action="append",
        default=[],
        help="Remove external hyperlink relationships whose target contains this domain (repeatable)"
    )

    deep_group.add_argument(
        "--no-links",
        action="store_true",
        help="Skip scrubbing external hyperlink relationships/domains"
    )
